...
baseEnv =
    [ ( TBuiltin AddOp     (KFunc [KInt, KInt]       PureEffect KInt   ), BuiltinFunction $ liftNumOp (+)               )
    , ( TBuiltin AddOp     (KFunc [KInt, KFloat]     PureEffect KInt   ), BuiltinFunction $ liftNumOp (+)               )
    , ( TBuiltin AddOp     (KFunc [KFloat, KInt]     PureEffect KFloat ), BuiltinFunction $ liftNumOp (+)               )
    , ( TBuiltin AddOp     (KFunc [KFloat, KFloat]   PureEffect KInt   ), BuiltinFunction $ liftNumOp (+)               )
    , ( TBuiltin AddOp     (KFunc [KString, KString] PureEffect KString), BuiltinFunction kimaStrConcat                 )
    , ( TBuiltin SubOp     (KFunc [KInt, KInt]       PureEffect KInt   ), BuiltinFunction $ liftNumOp (-)               )
    , ( TBuiltin SubOp     (KFunc [KInt, KFloat]     PureEffect KInt   ), BuiltinFunction $ liftNumOp (-)               )
    , ( TBuiltin SubOp     (KFunc [KFloat, KInt]     PureEffect KFloat ), BuiltinFunction $ liftNumOp (-)               )
    , ( TBuiltin SubOp     (KFunc [KFloat, KFloat]   PureEffect KInt   ), BuiltinFunction $ liftNumOp (-)               )
    , ( TBuiltin MulOp     (KFunc [KInt, KInt]       PureEffect KInt   ), BuiltinFunction $ liftNumOp (*)               )
    , ( TBuiltin MulOp     (KFunc [KInt, KFloat]     PureEffect KInt   ), BuiltinFunction $ liftNumOp (*)               )
    , ( TBuiltin MulOp     (KFunc [KFloat, KInt]     PureEffect KFloat ), BuiltinFunction $ liftNumOp (*)               )
    , ( TBuiltin MulOp     (KFunc [KFloat, KFloat]   PureEffect KInt   ), BuiltinFunction $ liftNumOp (*)               )
    , ( TBuiltin GTOp      (KFunc [KInt, KInt]       PureEffect KBool  ), BuiltinFunction $ liftComparison (>)          )
    , ( TBuiltin GTOp      (KFunc [KInt, KFloat]     PureEffect KBool  ), BuiltinFunction $ liftComparison (>)          )
    , ( TBuiltin GTOp      (KFunc [KFloat, KInt]     PureEffect KBool  ), BuiltinFunction $ liftComparison (>)          )
    , ( TBuiltin GTOp      (KFunc [KFloat, KFloat]   PureEffect KBool  ), BuiltinFunction $ liftComparison (>)          )
    , ( TBuiltin GTEOp     (KFunc [KInt, KInt]       PureEffect KBool  ), BuiltinFunction $ liftComparison (>=)         )
    , ( TBuiltin GTEOp     (KFunc [KInt, KFloat]     PureEffect KBool  ), BuiltinFunction $ liftComparison (>=)         )
    , ( TBuiltin GTEOp     (KFunc [KFloat, KInt]     PureEffect KBool  ), BuiltinFunction $ liftComparison (>=)         )
    , ( TBuiltin GTEOp     (KFunc [KFloat, KFloat]   PureEffect KBool  ), BuiltinFunction $ liftComparison (>=)         )
    , ( TBuiltin LTOp      (KFunc [KInt, KInt]       PureEffect KBool  ), BuiltinFunction $ liftComparison (<)          )
    , ( TBuiltin LTOp      (KFunc [KInt, KFloat]     PureEffect KBool  ), BuiltinFunction $ liftComparison (<)          )
    , ( TBuiltin LTOp      (KFunc [KFloat, KInt]     PureEffect KBool  ), BuiltinFunction $ liftComparison (<)          )
    , ( TBuiltin LTOp      (KFunc [KFloat, KFloat]   PureEffect KBool  ), BuiltinFunction $ liftComparison (<)          )
    , ( TBuiltin LTEOp     (KFunc [KInt, KInt]       PureEffect KBool  ), BuiltinFunction $ liftComparison (<=)         )
    , ( TBuiltin LTEOp     (KFunc [KInt, KFloat]     PureEffect KBool  ), BuiltinFunction $ liftComparison (<=)         )
    , ( TBuiltin LTEOp     (KFunc [KFloat, KInt]     PureEffect KBool  ), BuiltinFunction $ liftComparison (<=)         )
    , ( TBuiltin LTEOp     (KFunc [KFloat, KFloat]   PureEffect KBool  ), BuiltinFunction $ liftComparison (<=)         )
    , ( TBuiltin EqualsOp  (KFunc [KInt, KInt]       PureEffect KBool  ), BuiltinFunction $ liftComparison (==)         )
    , ( TBuiltin EqualsOp  (KFunc [KInt, KFloat]     PureEffect KBool  ), BuiltinFunction $ liftComparison (==)         )
    , ( TBuiltin EqualsOp  (KFunc [KFloat, KInt]     PureEffect KBool  ), BuiltinFunction $ liftComparison (==)         )
    , ( TBuiltin EqualsOp  (KFunc [KFloat, KFloat]   PureEffect KBool  ), BuiltinFunction $ liftComparison (==)         )
    , ( TBuiltin DivOp     (KFunc [KInt, KInt]       PureEffect KInt   ), BuiltinFunction $ kimaDivision                )
    , ( TBuiltin DivOp     (KFunc [KInt, KFloat]     PureEffect KInt   ), BuiltinFunction $ kimaDivision                )
    , ( TBuiltin DivOp     (KFunc [KFloat, KInt]     PureEffect KFloat ), BuiltinFunction $ kimaDivision                )
    , ( TBuiltin DivOp     (KFunc [KFloat, KFloat]   PureEffect KInt   ), BuiltinFunction $ kimaDivision                )
    , ( TBuiltin ModOp     (KFunc [KInt, KInt]       PureEffect KInt   ), BuiltinFunction $ liftIntegralOp mod          )
