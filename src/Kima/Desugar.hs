...
desugar (AccessE expr field) =
    Call (IdentifierE (Accessor field)) [desugar expr]

-- Basically just traverse
desugar (Program ast         ) = Program (desugar <$> ast)
desugar (DataDef name members) = DataDef name members
desugar (FuncDef name args eff rt body) =
    FuncDef name args eff rt (desugar body)
...
desugarBinary
    :: (TagSugar tag ~ 'NoSugar, NameAnnotation tag ~ 'NoAnnotation)
    => Binary (AST 'Expr tag)
    -> AST 'Expr tag
